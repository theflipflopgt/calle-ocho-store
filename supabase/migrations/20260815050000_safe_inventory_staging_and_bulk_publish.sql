-- Safe staging for mass inventory imports.
-- New products are ALWAYS created as archived, regardless of the Excel status.
-- Existing products keep their current visibility during stock/catalog updates.
-- Publishing is a separate admin-only action.
  ...
  LOOP
    ...
    ELSE
      UPDATE public.products SET
        ...
        -- Do not change visibility of an existing product during an import.
...
-- Publish selected staged products only after a human review.
-- This changes catalog visibility but does not alter stock, colors or variants.
CREATE OR REPLACE FUNCTION public.admin_publish_products(p_product_ids UUID[])
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  v_actor_id UUID := auth.uid();
  v_requested INTEGER := COALESCE(cardinality(p_product_ids), 0);
...
BEGIN
  ...
  IF v_requested < 1 OR v_requested > 100 THEN
    RAISE EXCEPTION 'INVALID_PRODUCT_SELECTION';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'ignored', v_requested - v_published
  );
END;
$$;

REVOKE ALL ON FUNCTION public.admin_publish_products(UUID[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_publish_products(UUID[]) TO authenticated;
